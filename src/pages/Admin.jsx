...
import {
  Calendar, Filter, RefreshCw, Check, X, Clock, User, Scissors, LayoutGrid, List,
} from 'lucide-react'
import {
  getAppointments, updateAppointmentStatus,
  getAllStaff, getServices,
} from '../lib/supabase'
...
function StatusBadge({ status }) {
  ...
  return (
    <span className="px-2.5 py-0.5 rounded-full text-xs font-medium" style={{ color: cfg.color, background: cfg.bg, fontFamily: "'DM Sans', sans-serif" }}>
      {cfg.label}
    </span>
  )
}
...
function AppointmentsTab({ appointments, onStatusChange, updating }) {
  ...
  const viewBtn = (id, Icon, label) => (
    <button
      ...
      style={{
        ...
        fontSize: '0.8rem',
        fontWeight: 500,
        background: view === id ? 'linear-gradient(135deg, #b76e79, #9b4f5e)' : 'rgba(183,110,121,0.1)',
        color: view === id ? 'white' : '#9b7a84',
    ...
    <div>
      ...
      <div className="glass-card rounded-2xl" style={{ padding: '1rem', marginBottom: '1.5rem', display: 'flex', flexWrap: 'wrap', gap: '0.75rem', alignItems: 'center' }}>
        <div style={{ display: 'flex', gap: '0.375rem' }}>
          ...
          {viewBtn('calendar', LayoutGrid, 'Calendar')}
        </div>

        {view === 'list' && (
          <>
            <div style={{ width: '1px', height: '1.25rem', background: 'rgba(183,110,121,0.2)' }} />
            <Filter size={14} color="#b76e79" />
      ...
      {/* Calendar view */}
      {view === 'calendar' && (
        <div className="glass-card rounded-2xl" style={{ padding: '1.5rem' }}>
      ...
      {view === 'list' && (
        filtered.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {filtered.map((appt, i) => (
              <div key={appt.id} className="glass-card rounded-2xl animate-fade-in-up" style={{ animationDelay: `${i * 0.04}s`, padding: '1.25rem' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '0.75rem', flexWrap: 'wrap' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', flex: 1, minWidth: 0 }}>
                    <div className="rounded-full font-semibold" style={{
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      <p className="text-xs" style={{ color: '#9b7a84', marginTop: '0.125rem', fontFamily: "'DM Sans', sans-serif" }}>
                        {appt.clients?.email} · {appt.clients?.phone}
                      </p>
                      <div style={{ color: '#7d5a62', display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginTop: '0.375rem', fontSize: '0.75rem', fontFamily: "'DM Sans', sans-serif" }}>
                        <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                          <Calendar size={11} /> <DateLabel dateStr={appt.appointment_date} />
                        </span>
                        <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                          <Clock size={11} /> {appt.start_time?.slice(0, 5)} – {appt.end_time?.slice(0, 5)}
                        </span>
                        {appt.staff && (
                          <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                            <User size={11} /> {appt.staff.name}
                          </span>
                        )}
                        <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                          <Scissors size={11} /> {appt.appointment_services?.map((s) => s.services?.name).join(', ') || '—'}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                {appt.notes && (
                  <p className="text-xs rounded-lg" style={{ marginTop: '0.75rem', padding: '0.5rem 0.75rem', background: 'rgba(183,110,121,0.06)', color: '#7d5a62', borderLeft: '2px solid rgba(183,110,121,0.3)', fontFamily: "'DM Sans', sans-serif" }}>
                    📝 {appt.notes}
                  </p>
                )}
              </div>
    ...
    <div className="gradient-mesh min-h-screen" style={{ width: '100%', paddingTop: '96px' }}>
      <div className="px-6 sm:px-10 lg:px-16" style={{ maxWidth: '80rem', marginLeft: 'auto', marginRight: 'auto', width: '100%', paddingTop: '3rem', paddingBottom: '3rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest" style={{ color: '#b76e79', fontFamily: "'DM Sans', sans-serif", marginBottom: '0.25rem' }}>
              Admin
            </p>
            <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(1.8rem, 3vw, 2.4rem)', color: '#3d2c35', fontWeight: 600 }}>
              Dashboard
            </h1>
          </div>
          <button onClick={fetchAll} className="rounded-full glass-card transition-colors" title="Refresh" style={{ color: '#9b7a84', padding: '0.625rem', display: 'flex' }}>
            <RefreshCw size={18} />
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4" style={{ gap: '1rem', marginBottom: '2rem' }}>
          {[
            ...
            <div key={label} className="glass-card rounded-2xl text-center" style={{ padding: '1rem' }}>
              <div className="rounded-full" style={{ width: '2.5rem', height: '2.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', marginLeft: 'auto', marginRight: 'auto', marginBottom: '0.5rem', background: `${color}18` }}>
                <Icon size={18} color={color} />
              </div>
              <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.8rem', fontWeight: 700, color: '#3d2c35' }}>{value}</div>
              <div className="text-xs" style={{ color: '#9b7a84', fontFamily: "'DM Sans', sans-serif" }}>{label}</div>
            </div>
          ))}
        </div>

        {/* Tab bar */}
        <div className="glass-card rounded-2xl" style={{ padding: '0.75rem 1rem', marginBottom: '1.5rem', display: 'flex', gap: '0.375rem', flexWrap: 'wrap' }}>
          {TABS.map((tab) => (
            <button
              key={tab.id}
              ...
              style={{
                ...
                fontSize: '0.875rem',
                fontWeight: 500,
                background: activeTab === tab.id ? 'linear-gradient(135deg, #b76e79, #9b4f5e)' : 'transparent',
                color: activeTab === tab.id ? 'white' : '#9b7a84',
                transition: 'all 0.15s',
              }}
            >
              {tab.label}
